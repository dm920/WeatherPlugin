...
from Plugins.Extensions.MSNWeather.plugin import weatherhandler
...
class MSNWeather(Source):
	...
	def __init__(self):
		...
		self.logo = self.services.get(config.plugins.MSNWeather.weatherservice.value, "msn")
		self.pluginpath = None
		self.iconpath = None

	def debug(self, text: str):
		if self.enabledebug:
			print("[MSNWeather] Source DEBUG %s" % text)
	...
	def getYahooCode(self, day: int):
		...
		if day == 0 and config.plugins.MSNWeather.nighticons.value and self.getIsNight() and iconcode in self.YAHOOnightswitch:
	...
	def getMeteoCode(self, day: int):
		...
		if day == 0 and config.plugins.MSNWeather.nighticons.value and self.getIsNight() and iconcode in self.METEOnightswitch:
