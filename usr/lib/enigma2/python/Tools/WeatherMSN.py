...
def write_log(msg):
    print("write_log wurde aufgerufen mit Nachricht:", msg)
    if logstatus == ('on'):
...
def logout(data):
    print("logout wurde aufgerufen mit Daten:", data)
    if logstatus == ('on'):
        write_log(data)
        return
    return
...
logout(data="start")
ablauf = "8888"
open("/tmp/main1.txt", "w").write(ablauf)
...
class Weatherinfo:
        ...
        if newmode in SOURCES:
            ...
            if self.mode != newmode:
                ...
                if newmode == "msn":
                    ...
                    if apikey:
                        logout(data="setmode  owm apikey")
                        self.parser = self.owmparser
                        logout(data=str(self.parser))
                    else:
                        logout(data="setmode  error apikey")
            ...
            logout(data="setmode  unknown mode")
    ...
    def getCitylist(self, cityname=None, scheme="de-de"):
        ...
        if not cityname:
            ...
            for city in [cityname, cityname.split(" ")[0]]:
                logout(data="getcitylist for city")
    ...
    def getreducedinfo(self):
        ...
        logout(data=str(self.parser))
        logout(data="getreducedinfo mode")
        logout(data=str(self.mode))
        if self.info:
            if self.parser is not None and self.mode == "msn":
                logout(data="getreducedinfo msn")
                if self.geodata:
                    try:
                ...
                if self.geodata:
                    ...
                    try:
                        ...
                        reduced["current"] = dict()
                        logout(data="getreducedinfo omw3")
                        for idx, time in enumerate(current["time"]):  # collect current
                            logout(data="getreducedinfo omw4")
                            if isotime in time:
                        ...
                        for idx in range(6):  # collect forecast of today and next 5 days
                            ...
                            logout(data="getreducedinfo omw10 currdate")
                ...
                if self.geodata:
                    try:
                        ...
                        now = datetime.now()
                        logout(data=str(now))
                        logout(data="getreducedinfo owm2 isotime")
                        isotime = datetime.now(timezone.utc).astimezone().isoformat()
                        logout(data=str(isotime))

                        reduced["current"]["observationPoint"] = self.geodata[0]
                        logout(data="getreducedinfo owm2a")
                        reduced["current"]["observationTime"] = "%s%s" % (isotime[: 19], isotime[26:])
                        ...
                        logout(data="getreducedinfo owm2d")
    ...
    def writereducedjson(self, filename):
        logout(data="919 writereducedjson ")
        self.error = None
    ...
    def showDescription(self, src):
        logout(data="1024 showdescription")
        self.error = None
        src = src.lower()
    ...
    def showConvertrules(self, src, dest):
        logout(data="1041 showconvertrules")
        self.error = None
        src = src.lower()
        dest = dest.lower()
        if not src:
...
def main(argv):
    ablauf = "1234"
    logout(data="main")
    open("/tmp/main.txt", "w").write(ablauf)
    mainfmt = "[__main__]"
    cityname = ""
    units = "metric"
    scheme = "de-de"
    ...
    if cityname:
        ...
        if citylist and len(citylist) > 1 and not quiet:
            ...
            if index > 0 and index < len(citylist) + 1:
                ...
                exit()
    if not specialopt:
        logout(data="main not specialopt")
        if geodata:
            logout(data="geodata wi strart")
            info = WI.start(geodata=geodata, units=units, scheme=scheme)  # INTERACTIVE CALL (unthreaded)
        elif cityID:
            logout(data="city id wi strart")
